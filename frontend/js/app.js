...
// Polls every 15s as safety net — WS is the primary real-time path
...
function renderItems() {
  ...
  container.innerHTML = state.currentItems.map((item, idx) => {
    ...
    // Suggest next minimum bid in placeholder
...
function _connectWS(roomId) {
  ...
  ws.onclose = (evt) => {
    ...
    // Auth failures — don't reconnect, force logout
    if (evt.code === 4001) {
      toast("Session expired — please login again", "error");
      logout();
      return;
    }
    // Room full — already handled by ERROR message
    if (evt.code === 4010) return;

    // Exponential backoff: 1s → 2s → 4s → 8s … max 30s
    const attempt = ++state._wsReconnectAttempt;
    const delay = Math.min(1000 * Math.pow(2, attempt - 1), 30000);
    addFeed(`Disconnected — reconnecting in ${(delay / 1000).toFixed(0)}s… (attempt ${attempt})`, "system");

    state._wsReconnectTimer = setTimeout(() => {
      if (!state._wsIntentionalClose && state.currentRoom?.id === roomId) {
        _connectWS(roomId);
      }
    }, delay);
  };
...
function setWsDot(status) {
  ...
  dot.title = status === "connected" ? "Connected" : status === "error" ? "Disconnected" : "Connecting…";
}
...
function wsSend(obj) {
  if (state.ws?.readyState === WebSocket.OPEN) {
    ...
    toast("Not connected — please wait", "error");
  }
}
...
function handleWsMessage(msg) {
  switch (msg.type) {
    ...
    case "POINTS_UPDATE": {
      ...
      updatePointsBar();
      // Re-render items to refresh the "available pts" line under bid inputs
      renderItems();
      break;
    }
    ...
    case "POINTS_RESET": {
      if (state.user?.role === "buyer") {
        // Optimistic update — exact value arrives via POINTS_UPDATE shortly after
        ...
        updatePointsBar();
        renderItems();
        // Re-fetch leaderboard so ranking reflects reset
    ...
    case "PARTICIPANT_COUNT": {
      // Could update a live player counter in the header here if desired
      break;
    }
...
function startCountdown(itemId, seconds) {
  ...
  const interval = setInterval(() => {
    ...
    text.textContent = `${remaining}s — bid now to outbid`;
    // Urgent ticking in last 3 seconds
...
function withdrawBid(itemId) {
  ...
  if (!confirm(`Withdraw your bid of ${formatCurrency(myBid.amount)}? Points will be refunded immediately.`)) return;
...
function renderLeaderboard() {
  ...
  if (!top5.length) {
    list.innerHTML = `<div style="color:var(--text-mute);font-family:var(--mono);font-size:12px;">Waiting for bids...</div>`;
    return;
  }
