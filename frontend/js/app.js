...
function updatePointsBar() {
  const wrap = $("points-bar-wrap"), fill = $("points-bar-fill");
  if (!wrap || !fill) return;
  if (!state.user || state.user.role === "admin" || state.user.isSpectator) { wrap.style.display = "none"; return; }
  wrap.style.display = "block";
  const pts = parseFloat(state.user.points || 0);
  fill.style.width = `${Math.min(100, Math.max(0, (pts / 10000) * 100))}%`;
...
function renderNavUser() {
  const wrap = $("nav-user-wrap"); if (!wrap) return;
  ...
  const pointsHTML = state.user.role !== "admin"
    ...
    : "";
  wrap.innerHTML = `${pointsHTML}<span class="nav-user">${escapeHTML(state.user.name)}</span><span class="nav-badge ${state.user.role === "admin" ? "admin" : ""}">${state.user.role.toUpperCase()}</span><button class="btn btn-ghost" onclick="logout()" style="padding:6px 12px">LOGOUT</button>`;
  updatePointsBar();
}
...
async function handleAuthSubmit(e) {
  ...
  try {
    ...
    saveAuth(data.token, data.user); renderNavUser(); showLobby();
  } catch (err) {
...
async function showLobby() {
  stopLeaderboardPolling(); showPage("page-lobby");
  $("create-room-btn").style.display = state.user?.role === "admin" ? "inline-flex" : "none";
  await loadRooms();
}
async function loadRooms() {
  try { renderRooms((await apiCall("GET", "/rooms")).rooms); }
...
function renderRooms(rooms) {
  ...
  grid.innerHTML = rooms.map(r => {
    ...
    return `<div class="room-card" onclick="joinRoom(${r.id})">
      ${state.user?.role === "admin" ? `<div style="position:absolute;top:12px;right:12px;font-size:16px;cursor:pointer;z-index:1;" onclick="event.stopPropagation();deleteRoomReq(${r.id},'${escapeHTML(r.name)}','${r.status}')">🗑️</div>` : ""}
      ...
      <div class="room-meta">
        ...
        ${r.max_players ? `<span>👥 Max ${r.max_players}</span>` : ""}
      </div>
...
async function deleteRoomReq(id, name, status) {
  if (!confirm(status === "active" ? `⚠️ "${name}" is active! Delete anyway?` : `Delete "${name}"?`)) return;
...
async function submitCreateRoom() {
  ...
  try { await apiCall("POST", "/rooms", { name, max_players }); closeCreateRoomModal(); toast("Room created!", "success"); await loadRooms(); }
  catch (err) { toast(err.message, "error"); }
}
...
async function joinRoom(roomId) {
  try {
    ...
    state.currentRoom = data.room; state.currentItems = data.items;
    state.topBids = {}; state.activeBidders = {};
    data.topBids.forEach(b => { state.topBids[b.item_id] = { userId: b.user_id, userName: b.user_name, amount: b.amount }; });
    state.allBids = {};
    if (data.allBids) data.allBids.forEach(b => { if (!state.allBids[b.item_id]) state.allBids[b.item_id] = []; state.allBids[b.item_id].push(b); });
    state.resultsRevealed = data.room.status === "finished";
    renderArena(); showPage("page-arena");
    connectWebSocket(roomId); startLeaderboardPolling(roomId);
...
function renderItems() {
  const container = $("items-container"); if (!container) return;
  ...
  container.innerHTML = state.currentItems.map((item, idx) => {
    ...
    const isActive = item.status === "active", isFinished = item.status === "finished", isAdmin = state.user?.role === "admin";
    ...
    if (revealed && isFinished) {
      ...
      if (myBid) {
        ...
        pnlHTML = `<div class="${diff >= 0 ? "profit" : "loss"}" style="font-family:var(--mono);font-size:13px;margin-top:4px;">Your bid: ${formatCurrency(myBid)} → Actual: ${formatCurrency(actual)} → P&L: <span>${diff >= 0 ? "+" : ""}${formatCurrency(diff)}</span></div>`;
      } else {
    ...
    if (isFinished && item.winner_id) {
      const wn = item.winner_name || state.leaderboard.find(l => String(l.id) === String(item.winner_id))?.name || (String(item.winner_id) === String(state.user?.id) ? state.user.name : "Unknown");
      const isMyWin = String(item.winner_id) === String(state.user?.id);
      winnerHTML = `<div style="margin-top:12px;padding:10px 14px;background:rgba(245,197,24,0.06);border:1px solid rgba(245,197,24,0.25);border-radius:2px;font-family:var(--mono);font-size:13px;">🏆 Won by <strong style="color:var(--gold);">${escapeHTML(wn)}</strong>${isMyWin ? ' <span style="color:var(--green);">(You!)</span>' : ''} for ${formatCurrency(item.winning_bid)}</div>`;
    ...
    const liveBiddersHTML = isActive && bidders.length > 0 ? `
      <div class="live-bidders">
        ...
        ${bidders.map((b, i) => { const isMe = String(b.userId) === String(state.user?.id); return `<div class="live-bidder-row"><span class="live-bidder-name">${i === 0 ? "👑 " : ""}${escapeHTML(b.userName)}${isMe ? '<span class="live-bidder-you">(you)</span>' : ""}</span><span class="live-bidder-amount">${formatCurrency(b.amount)}</span></div>`; }).join("")}
      </div>` : "";

    const nextMin = topBid ? (parseFloat(topBid.amount) + 1).toFixed(0) : "100";

    return `<div class="item-card ${isActive ? "active-item" : ""} ${isFinished ? "finished-item" : ""}" id="item-card-${item.id}">
      ...
      <div class="bid-info">
        <div class="bid-block">
          <div class="bid-label">${isActive ? "Leading Bid" : isFinished ? "Winning Bid" : "Current High"}</div>
      ...
      <div class="countdown-wrap" id="countdown-${item.id}">
        ...
        <div class="countdown-text" id="countdown-text-${item.id}">bid now to outbid</div>
      </div>
      ${liveBiddersHTML}${winnerHTML}${pnlHTML}
      ${isActive && !isAdmin ? (state.user?.isSpectator ? `<div style="margin-top:16px;"><div class="spectating-badge">👀 SPECTATING</div></div>` : `
        <div class="bid-row" id="bid-row-${item.id}" style="margin-top:14px;">
          <div class="bid-input-wrap">
            <span class="bid-currency">₹</span>
            <input class="form-input" type="number" id="bid-val-${item.id}" placeholder="${nextMin}" min="${nextMin}" step="1" onkeydown="if(event.key==='Enter') placeBid(${item.id})" />
          </div>
          <button class="btn btn-gold" id="bid-btn-${item.id}" onclick="placeBid(${item.id})" ${isHighestBidder ? "disabled" : ""}>${isHighestBidder ? "LEADING" : "PLACE BID"}</button>
          ${myActiveBid ? `<button class="btn btn-withdraw" id="withdraw-btn-${item.id}" onclick="withdrawBid(${item.id})" title="Withdraw bid of ${formatCurrency(myActiveBid.amount)}">✕ WITHDRAW ${formatCurrency(myActiveBid.amount)}</button>` : ""}
        </div>
        <div style="font-family:var(--mono);font-size:11px;color:var(--text-mute);margin-top:6px;">Available: <span style="color:var(--gold);">⚡ ${parseFloat(state.user?.points || 0).toFixed(0)} pts</span></div>
      `) : ""}
      ${isFinished && state.allBids[item.id]?.length > 0 ? `
        <div class="bid-history">
          ...
          <div class="bid-history-list" id="bid-hist-${item.id}">
            ${state.allBids[item.id].map(b => `<div class="bid-history-item"><span>${escapeHTML(b.user_name)}</span><span>${formatCurrency(b.amount)}</span></div>`).join("")}
          </div>
        </div>` : ""}
...
function connectWebSocket(roomId) {
  closeWebSocket(); state._wsIntentionalClose = false; state._wsReconnectAttempt = 0;
  _connectWS(roomId);
}
...
function handleWsMessage(msg) {
  switch (msg.type) {
    case "ROOM_SNAPSHOT": {
      state.currentRoom = msg.data.room; state.currentItems = msg.data.items;
      if (msg.data.isSpectator !== undefined) { state.user.isSpectator = msg.data.isSpectator; updatePointsBar(); }
      msg.data.topBids.forEach(b => { state.topBids[b.item_id] = { userId: b.user_id, userName: b.user_name || "?", amount: b.amount }; });
      if (msg.data.activeBidderMap) { state.activeBidders = {}; for (const [id, bids] of Object.entries(msg.data.activeBidderMap)) state.activeBidders[id] = bids; }
      renderItems(); break;
    }
    case "REAUTH_REQUIRED": toast(msg.reason + " — Please login again.", "error"); setTimeout(logout, 3000); break;
    case "PONG": break;
    case "BID_PLACED": {
    ...
    case "BIDDER_LIST": {
      ...
      if (bidders.length > 0) { const top = bidders[0]; state.topBids[itemId] = { userId: top.userId, userName: top.userName, amount: top.amount }; updateBidDisplay(itemId, state.topBids[itemId]); }
      renderItems(); break;
    }
    case "BID_WITHDRAWN": {
      const { itemId, userId, userName } = msg.data;
      addFeed(`<b>${escapeHTML(userName)}</b> withdrew their bid`, "system");
      if (String(userId) === String(state.user?.id)) toast("Your bid was withdrawn and points refunded", "success");
      break;
    }
    case "LAST_BIDDER_REMAINING": {
      const { itemId, userName, amount, graceSecs } = msg.data;
      addFeed(`⏳ Last bidder: <b>${escapeHTML(userName)}</b> @ ${formatCurrency(amount)} — closes in ${graceSecs}s`, "bid");
      startCountdown(itemId, graceSecs); break;
    }
    case "BID_WINDOW_OPEN": {
      const { itemId, userId, userName, amount, expiresIn, bidding_window_started_at } = msg.data;
      state.bidWindowActive = true; state.bidWindowUserId = userId; state.activeItemId = itemId;
      updateBidDisplay(itemId, { userId, userName, amount });
      let remaining = expiresIn;
      if (bidding_window_started_at) remaining = Math.max(1, expiresIn - (Date.now() - bidding_window_started_at) / 1000);
      startCountdown(itemId, remaining); refreshBidButton(itemId, userId);
    ...
    case "BID_COMMITTED": {
      ...
      state.bidWindowActive = false; stopCountdown(itemId);
      updateBidDisplay(itemId, { userId, userName, amount }); refreshBidButton(itemId, userId);
      addFeed(`✓ Confirmed: <b>${escapeHTML(userName)}</b> @ ${formatCurrency(amount)}`, "system");
      if (String(userId) === String(state.user?.id)) toast(`Your bid of ${formatCurrency(amount)} is leading`, "success");
      break;
    }
    case "POINTS_UPDATE": {
      state.user.points = msg.data.points; localStorage.setItem("ba_user", JSON.stringify(state.user));
      const navPts = $("nav-points"); if (navPts) navPts.textContent = `⚡ ${parseFloat(msg.data.points).toFixed(0)} pts`;
      updatePointsBar(); renderItems(); break;
    }
    case "POINTS_RESET": {
      if (state.user?.role === "buyer") {
        state.user.points = 10000; localStorage.setItem("ba_user", JSON.stringify(state.user));
        toast("Points reset to 10,000 by admin!", "gold");
        const navPts = $("nav-points"); if (navPts) navPts.textContent = `⚡ 10000 pts`;
        updatePointsBar(); renderItems();
        if (state.currentRoom) apiCall("GET", `/rooms/${state.currentRoom.id}/leaderboard`).then(d => { if (d.leaderboard) { state.leaderboard = d.leaderboard; renderLeaderboard(); } }).catch(() => {});
      }
      break;
    }
    case "BID_REJECTED": playSound("error"); toast(msg.reason, "error"); break;
    case "ITEM_STARTED": {
      updateItemInState(msg.data); state.activeBidders[msg.data.id] = []; renderItems();
      addFeed(`📦 Bidding started: <b>${escapeHTML(msg.data.name)}</b>`, "system");
      toast(`Bidding open: ${msg.data.name}`, "gold"); break;
    }
    case "ITEM_ENDED": {
      const item = msg.data; stopCountdown(item.id); state.bidWindowActive = false;
      delete state.activeBidders[item.id]; updateItemInState(item); renderItems();
      if (item.winner_name) {
        const isMyWin = String(item.winner_id) === String(state.user?.id);
        addFeed(`🏆 <b>${escapeHTML(item.name)}</b> → <b>${escapeHTML(item.winner_name)}</b> for ${formatCurrency(item.winning_bid)}`, "win");
        showWinnerBanner(item.winner_name, item.name, item.winning_bid); playSound("success");
        ...
        addFeed(`🔔 <b>${escapeHTML(item.name)}</b> — no bids, unsold`, "system");
        toast(`Bidding ended for ${item.name} — no winner`, "info");
      }
      break;
    }
    case "ITEM_UPDATE": updateItemInState(msg.data); renderItems(); break;
    case "LEADERBOARD": state.leaderboard = msg.data; renderLeaderboard(); break;
    case "PRICES_REVEALED":
      msg.data.items.forEach(i => updateItemInState(i));
      state.resultsRevealed = true; if (state.currentRoom) state.currentRoom.status = "finished";
      renderArena(); toast("🎉 Prices revealed! Check your results.", "gold");
      addFeed("🏁 GAME OVER — Prices revealed!", "win"); break;
    case "USER_JOINED": addFeed(`👤 ${escapeHTML(msg.data.name)} joined`, "system"); break;
    case "PARTICIPANT_COUNT": break;
    case "ERROR":
      toast(msg.reason || "An error occurred", "error");
      if (msg.reason?.includes("full")) setTimeout(() => showLobby(), 2000); break;
  }
}
...
function updateBidDisplay(itemId, { userId, userName, amount }) {
  const amtEl = $(`bid-amount-${itemId}`), userEl = $(`bid-user-${itemId}`);
...
function refreshBidButton(itemId, highBidderUserId) {
  const btn = $(`bid-btn-${itemId}`); if (!btn) return;
  const isMe = String(highBidderUserId) === String(state.user?.id);
  btn.disabled = isMe; btn.textContent = isMe ? "LEADING" : "PLACE BID";
}

// ─── COUNTDOWN ─────────────────────────────────
function startCountdown(itemId, seconds) {
  const wrap = $(`countdown-${itemId}`), fill = $(`countdown-fill-${itemId}`), text = $(`countdown-text-${itemId}`);
  if (!wrap) return;
  stopCountdown(itemId); wrap.classList.add("visible");
  let remaining = Math.ceil(seconds);
  fill.style.width = "100%"; text.textContent = `${remaining}s — bid now to outbid`;
  const interval = setInterval(() => {
    remaining--;
    if (remaining <= 0) { stopCountdown(itemId); return; }
    fill.style.width = `${(remaining / Math.ceil(seconds)) * 100}%`;
...
function stopCountdown(itemId) {
  if (state.countdownInterval?.id === itemId) { clearInterval(state.countdownInterval.interval); state.countdownInterval = null; }
  const wrap = $(`countdown-${itemId}`); if (wrap) wrap.classList.remove("visible");
}

// ─── BIDDING ACTIONS ───────────────────────────
function placeBid(itemId) {
  const input = $(`bid-val-${itemId}`), amount = parseFloat(input?.value);
  if (!amount || amount <= 0) return toast("Enter a valid bid amount", "error");
  const topBid = state.topBids[itemId];
  if (topBid && amount <= parseFloat(topBid.amount)) return toast(`Bid must exceed ${formatCurrency(topBid.amount)}`, "error");
...
function adminEndItem(itemId) {
  const item = state.currentItems.find(i => i.id === itemId), topBid = state.topBids[itemId];
  if (!confirm(topBid ? `End "${item?.name}"?\n\nWinner: ${topBid.userName} @ ${formatCurrency(topBid.amount)}` : `End "${item?.name}"? No bids — will be unsold.`)) return;
  wsSend({ type: "ADMIN_END_ITEM", itemId });
}
function adminRevealPrices() {
  if (!confirm("Reveal actual prices?\n\nThis ends the game and sends result emails to all participants.")) return;
...
async function openAdminDashboard() {
  ...
  $("dash-active-item").textContent = state.currentItems.find(i => i.status === "active")?.name || "None";
  const mp = state.currentRoom?.max_players;
  const dashMp = $("dash-max-players"); if (dashMp) dashMp.textContent = mp ? `${leaderboard.length} / ${mp}` : `${leaderboard.length} (no limit)`;
  const list = $("dash-players-list"), maxPts = Math.max(...leaderboard.map(p => parseFloat(p.net_worth)), 1);
  list.innerHTML = leaderboard.length ? leaderboard.map(p => {
    const pts = parseFloat(p.net_worth), pct = Math.max(0, (pts / maxPts) * 100);
    return `<div style="margin-bottom:8px;"><div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span style="font-family:var(--mono);font-size:13px;">${escapeHTML(p.name)}</span><span style="color:var(--gold);font-family:var(--mono);font-size:13px;">⚡ ${pts.toFixed(0)} pts</span></div><div style="height:4px;background:var(--bg4);border-radius:2px;overflow:hidden;"><div style="height:100%;width:${pct}%;background:var(--gold);transition:width .3s;"></div></div></div>`;
...
async function adminResetPoints() {
  if (!confirm("Reset ALL buyers to 10,000 points?\n\nAll connected players will be updated instantly.")) return;
  try {
    await apiCall("POST", "/rooms/reset-points");
    toast("All buyers reset to 10,000 points!", "success"); closeAdminDashboard();
    if (state.currentRoom) setTimeout(async () => { const d = await apiCall("GET", `/rooms/${state.currentRoom.id}/leaderboard`); if (d.leaderboard) { state.leaderboard = d.leaderboard; renderLeaderboard(); } }, 500);
  } catch (err) { toast(err.message, "error"); }
}
...
function closeAddItemModal() {
  ...
  ["item-name-input", "item-desc-input", "item-price-input", "item-order-input"].forEach(id => { const el = $(id); if (el) el.value = ""; });
}
async function submitAddItem() {
  const name = $("item-name-input").value.trim(), actual_price = parseFloat($("item-price-input").value);
  if (!name) return toast("Item name required", "error");
  if (!actual_price || isNaN(actual_price)) return toast("Actual price required", "error");
  try {
    await apiCall("POST", `/rooms/${state.currentRoom.id}/items`, { name, description: $("item-desc-input").value.trim(), actual_price, display_order: parseInt($("item-order-input").value) || (state.currentItems.length + 1) });
    closeAddItemModal(); toast("Item added!", "success");
    state.currentItems = (await apiCall("GET", `/rooms/${state.currentRoom.id}`)).items; renderItems();
  } catch (err) { toast(err.message, "error"); }
}
...
function renderLeaderboard() {
  ...
  if (!top5.length) { list.innerHTML = `<div style="color:var(--text-mute);font-family:var(--mono);font-size:12px;">No players yet</div>`; return; }
  list.innerHTML = top5.map((p, i) => {
    ...
    return `<div class="lb-entry ${rankClass}" style="${isMe ? "border-color:var(--green);" : ""}">
      ...
      <div class="lb-score"><div class="lb-won">${p.items_won} won</div><div style="color:var(--gold);">⚡ ${parseFloat(p.net_worth).toFixed(0)}</div></div>
    </div>`;
  }).join("");
}
...
function renderResults() {
  ...
  const itemsHTML = finishedItems.map(item => {
    const actual = parseFloat(item.actual_price || 0), winning = parseFloat(item.winning_bid || 0);
    const isMyWin = String(item.winner_id) === String(state.user?.id);
    const diff = actual - winning;
    const wn = item.winner_name || state.leaderboard.find(l => String(l.id) === String(item.winner_id))?.name || (isMyWin ? state.user.name : "Unknown");
    return `<div class="result-item">
      <div class="result-item-name">${escapeHTML(item.name)}</div>
      <div class="result-row"><span>Winner</span><span>${item.winner_id ? (isMyWin ? "YOU 🏆" : escapeHTML(wn)) : "—"}</span></div>
      ...
      ${isMyWin ? `<div class="result-pnl ${diff >= 0 ? "profit" : "loss"}">${diff >= 0 ? "+" : ""}${formatCurrency(diff)} P&L</div>` : ""}
    </div>`;
  }).join("");
  ...
  const summaryHTML = myEntry ? `<div class="card" style="margin-top:0"><div class="card-title">YOUR SUMMARY</div><div class="card-sub" style="margin-bottom:16px">FINAL STANDINGS</div><div style="display:flex;flex-direction:column;gap:10px;font-family:var(--mono);font-size:14px;"><div style="display:flex;justify-content:space-between;"><span style="color:var(--text-dim)">Items Won</span><span style="color:var(--gold)">${myEntry.items_won}</span></div><div style="display:flex;justify-content:space-between;"><span style="color:var(--text-dim)">Total Spent</span><span>${formatCurrency(myEntry.total_spent)}</span></div><div style="display:flex;justify-content:space-between;border-top:1px solid var(--border);padding-top:10px;"><span style="color:var(--text-dim)">Rank</span><span style="color:var(--gold);">#${myRank}</span></div></div></div>` : "";
  wrap.innerHTML = `<div style="padding:32px;border-top:1px solid var(--border);"><div class="results-title">RESULTS</div><div class="results-sub">ACTUAL PRICES REVEALED BY ADMIN</div><div class="results-grid"><div class="results-items">${itemsHTML}</div><div>${summaryHTML}</div></div></div>`;
}

// ─── BACK TO LOBBY ─────────────────────────────
function backToLobby() {
  closeWebSocket(); stopLeaderboardPolling();
  state.currentRoom = null; state.currentItems = []; state.topBids = {}; state.activeBidders = {};
  showLobby();
}

// ─── INIT ──────────────────────────────────────
document.addEventListener("DOMContentLoaded", () => {
  if (state.token && state.user) { renderNavUser(); showLobby(); }
  else showAuthPage();
});